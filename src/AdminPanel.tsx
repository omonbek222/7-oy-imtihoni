  ...
  return (
    <div className={`p-6 min-h-full ${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { title: 'Umumiy foydalanuvchilar', count: '1,245', desc: "So'nggi oy statistikasi" },
          { title: 'Yangi registratsiyalar', count: '87', desc: 'Oxirgi 7 kun ichida' },
          { title: 'Faol foydalanuvchilar', count: '562', desc: 'Bugungi holat' },
        ].map((item, i) => (
          <div
            key={i}
            className={`p-6 rounded-lg shadow-lg transition-shadow ${
              darkMode ? 'bg-[#1f1f1f] hover:shadow-indigo-600' : 'bg-gray-200 hover:shadow-indigo-300'
            }`}
          >
            <h2 className="text-xl font-semibold mb-2">{item.title}</h2>
            <p className="text-3xl font-bold">{item.count}</p>
            <p className={`${darkMode ? 'text-gray-400' : 'text-gray-600'} mt-1`}>{item.desc}</p>
          </div>
        ))}
      </div>
